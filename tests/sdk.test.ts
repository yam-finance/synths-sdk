...
beforeAll(async () => {
    ...
    const provider: ExternalProvider = (new Web3Provider(web3.currentProvider as ExternalProvider)).provider;
    ...
    UGAS0221 = await new Asset(SDK.synthetics["ugas"]["0221"].asset, SDK.options);
});
...
describe('General function testing', () => {
    ...
    it("getUserBalanceWETH must return false when no account provided", async () => {
        const noUserSDK = await new Synths({
            provider: SDK.provider,
            network: SDK.network,
        });
        const getUserBalanceWETH = await noUserSDK.methods.getUserBalanceWETH()
        expect(typeof getUserBalanceWETH).toBe("boolean");
        expect(getUserBalanceWETH).toBe(false)
    });
